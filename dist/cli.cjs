...
var import_form_data, import_fs, import_path, FormDataHandler, form_data_handler_default;
...
var import_stream, import_fs2, import_promises, StreamProcessors, stream_processors_default;
  ...
  "lib/stream-processors.js"() {
    ...
    StreamProcessors = class {
      ...
      async streamToFile(dataStream, filePath, format = "json", options = {}) {
        const writeStream = (0, import_fs2.createWriteStream)(filePath, { encoding: "utf8" });
        try {
...
async function createDataStream(config) {
  ...
  if (typeof data === "string") {
    if ((0, import_fs3.existsSync)(import_path2.default.resolve(data))) {
      ...
      if (fileExt === ".json") {
        ...
        return (0, import_fs3.createReadStream)(filePath, { highWaterMark }).pipe(streamProcessors.createJSONLTransform());
      }
    } else if ((0, import_ak_tools2.isJSONStr)(data)) {
      const parsed = JSON.parse(data);
      return import_stream2.Readable.from(Array.isArray(parsed) ? parsed : [parsed]);
    } else if (data.split("\n").every((line) => line.trim() === "" || (0, import_ak_tools2.isJSONStr)(line))) {
...
async function processDataStream(stream, config, logger) {
  ...
  stream.on("end", () => {
    if (logger.isVerbose()) {
      logger.info(`Stream ended: processed ${(0, import_ak_tools2.comma)(rowCount)} records, ${(0, import_ak_tools2.comma)(reqCount)} requests`);
    }
  });
...
async function writeLogFile(filePath, data, format, verbose) {
  try {
    await (0, import_ak_tools2.makeExist)(filePath);
...
var import_run_queue, import_ak_tools2, import_child_process, import_stream2, import_path2, import_fs3, import_dotenv, import_meta, index_default;
var init_index = __esm({
  "index.js"() {
    "use strict";
    import_run_queue = __toESM(require("run-queue"), 1);
    import_ak_tools2 = require("ak-tools");
    init_cli();
    ...
    if (import_meta.url === `file://${process.argv[1]}`) {
      cli_default().then((params) => {
        ...
        if (params.verbose) {
          ...
          logger.log((0, import_ak_tools2.json)(results));
        }
      }).catch((error) => {
        const logger = createLogger({ verbose: true });
...
var import_yargs, import_fs4, import_ak_tools3, import_meta2, packageJson, version, hero, banner, welcome, cli_default;
var init_cli = __esm({
  "cli.js"() {
    import_yargs = __toESM(require("yargs"), 1);
    import_fs4 = require("fs");
    import_ak_tools3 = __toESM(require("ak-tools"), 1);
    init_logger();
    import_meta2 = {};
    packageJson = JSON.parse((0, import_fs4.readFileSync)("./package.json", "utf8"));
    ({ version } = packageJson);
    hero = String.raw`
░▒▓██████▓▒░░▒▓█▓▒░░▒▓█▓▒░      ░▒▓████████▓▒░▒▓████████▓▒░▒▓████████▓▒░▒▓██████▓▒░░▒▓█▓▒░░▒▓█▓▒░ 
░▒▓█▓▒░░▒▓█▓▒░▒▓█▓▒░░▒▓█▓▒░      ░▒▓█▓▒░      ░▒▓█▓▒░         ░▒▓█▓▒░  ░▒▓█▓▒░░▒▓█▓▒░▒▓█▓▒░░▒▓█▓▒░ 
